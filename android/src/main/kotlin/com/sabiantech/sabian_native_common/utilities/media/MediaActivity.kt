...
import com.sabiantech.sabian_native_common.channels.methods.photo.PhotoConfig
import com.sabiantech.sabian_native_common.extensions.toColorOrNull
...
abstract class MediaActivity : FlutterActivity() {
    ...
    fun takePicture(resCode: Int, picAction: (Uri, File) -> Unit, onError: ((Throwable) -> Unit)? = null) {
        ...
        per.proceedIfPhotoPermissionsGranted({

            if (it.isAnyPermissionDenied) {
                onError?.invoke(SabianException(getString(R.string.please_accept_all_permissions)))
